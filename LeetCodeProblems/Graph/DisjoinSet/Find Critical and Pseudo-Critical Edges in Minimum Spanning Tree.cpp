...
public:
    vector<vector<int>> findCriticalAndPseudoCriticalEdges(int n, vector<vector<int>>& edges) {
        ...
        for (int i = 0; i < m; ++i) {
            ...
            {
                dsu.connect_fast(edges[i][0], edges[i][1]);
                mstWeight += edges[i][2];
            }
        }
        ...
        for (int i = 0; i < m; ++i) {
            DisjoinSet tempDsu(n);
            int weightWithout = 0;
            for (int j = 0; j < m; ++j) {
                if (i != j && !tempDsu.are_connect(edges[j][0], edges[j][1]))
                {
                    tempDsu.connect_fast(edges[j][0], edges[j][1]);
            ...
            if (tempDsu.get_sets() != 1 || weightWithout > mstWeight) critical.push_back(edges[i][3]);
                ...
                DisjoinSet newDsu(n);
                newDsu.connect_fast(edges[i][0], edges[i][1]);
                int weightWith = edges[i][2];
                for (int j = 0; j < m; ++j) {
                    if (i != j && !newDsu.are_connect(edges[j][0], edges[j][1]))
                    {
                        newDsu.connect_fast(edges[j][0], edges[j][1]);
